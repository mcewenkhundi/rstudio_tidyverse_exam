...
```{r import data}
medians <- read_csv("https://education.rstudio.com/blog/2020/02/instructor-certification-exams/at_health_facilities.csv",
                na = c("-", "")) %>% 
  janitor::clean_names() %>% 
  group_by(iso3, year) %>% 
...
my_iso3 <- params$my_country # can I make this a parameter?
single_iso <- medians %>% 
  filter(iso3 == my_iso3) 
